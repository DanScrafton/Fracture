...
public class Voronoi_Calculator : MonoBehaviour
{
    Delaunay_Calculator delCalc;
    PTComparer cmp;
    ...
    public Voronoi_Calculator()
    {
        pts = new List<PointTriangle>();
        delCalc = new Delaunay_Calculator();
        cmp = new PTComparer();
    }
    ...
    /// </summary>
    public Voronoi_Diagram CalculateDiagram(IList<Vector2> inputVertices)
    {
        Voronoi_Diagram result = null;
        Calculate_Diagram(inputVertices, ref result);
        return result;
    }
    ...
    /// </summary>
    public void Calculate_Diagram(IList<Vector2> inputVertices, ref Voronoi_Diagram result)
    {
        ...
        delCalc.Calculate_Triangulation(inputVertices, ref trig);
        Profiler.EndSample();
        ...
        for (int ti = 0; ti < tris.Count; ti += 3)
        {
            ...
            Debug.Assert(Geometry.To_The_Left(p2, p0, p1));

            centers.Add(Geometry.Circumcircle_Center(p0, p1, p2));
        }
        ...
        for (int i = 0; i < pts.Count; i++)
        {
            ...
            for (int ptiCurr = start; ptiCurr <= end; ptiCurr++)
            {
                ...
                else if (count == 1)
                {
                    ...
                    var cCurr = Geometry.Triangle_Centroid(verts[tris[tiCurr]], verts[tris[tiCurr + 1]], verts[tris[tiCurr + 2]]);
                    var cNext = Geometry.Triangle_Centroid(verts[tris[tiNext]], verts[tris[tiNext + 1]], verts[tris[tiNext + 2]]);

                    isEdge = Geometry.To_The_Left(cCurr, p0, cNext);
                }
                else
                {
                    isEdge = !Shares_Edge(tris, tiCurr, tiNext);
                }
                ...
                if (isEdge)
                {
                    ...
                    edges.Add(new Voronoi_Diagram.Edge(
                        Voronoi_Diagram.EdgeType.RayCCW,
                        ptCurr.Point,
                        tiCurr / 3,
                        -1,
                        Geometry.Rotate_RightAngle(v0)
                    ));

                    edges.Add(new Voronoi_Diagram.Edge(
                        Voronoi_Diagram.EdgeType.RayCW,
                        ptCurr.Point,
                        tiNext / 3,
                        -1,
                        Geometry.Rotate_RightAngle(v1)
                    ));
                }
                else
                {
                    if (!Geometry.AreCoincident(centers[tiCurr / 3], centers[tiNext / 3]))
                    {
                        edges.Add(new Voronoi_Diagram.Edge(
                            Voronoi_Diagram.EdgeType.Segment,
                            ptCurr.Point,
                            tiCurr / 3,
                            tiNext / 3,
                            v2nan
    ...
    /// ti1?
    /// <summary>
    static int Non_Shared_Point(List<int> tris, int ti0, int ti1)
    {
        Debug.Assert(Shares_Edge(tris, ti0, ti1));
    ...
    static bool Shares_Edge(List<int> tris, int ti0, int ti1)
    {
        var x0 = tris[ti0];
        var x1 = tris[ti0 + 1];
        var x2 = tris[ti0 + 2];
    ...
    class PTComparer : IComparer<PointTriangle>
    {
        ...
        {
            var ti = pt.Triangle;
            return Geometry.Triangle_Centroid(verts[tris[ti]], verts[tris[ti + 1]], verts[tris[ti + 2]]);
        }
    }
}
