...
public class Delaunay_Calculator : MonoBehaviour
{
    int highest = -1;
    IList<Vector2> verts;
    ...
    public Delaunay_Triangulation Calculate_Triangulation(IList<Vector2> verts)
    {
        Delaunay_Triangulation result = null;
        Calculate_Triangulation(verts, ref result);
    ...
    public void Calculate_Triangulation(IList<Vector2> verts, ref Delaunay_Triangulation result)
    {
        if (verts == null)
        {
    ...
    void Generate_Result(ref Delaunay_Triangulation result)
    {
        if (result == null)
        {
            result = new Delaunay_Triangulation();
        }
    ...
    bool LegalEdge(int k, int l, int i, int j)
    {
        ...
        else if (iMagic)
        {
            ...
            return Geometry.To_The_Left(p, l0, l1);
        }
        else if (jMagic)
        {
            Debug.Assert(!iMagic);
        ...
        else
        {
            ...
            Debug.Assert(Geometry.To_The_Left(c2, c0, c1));
            Debug.Assert(Geometry.To_The_Left(c2, c1, p));

            return !Geometry.Inside_Circumcircle(p, c0, c1, c2);
        }
    }
    ...
    int FindTriangleNode(int pi)
    {
        ...
        while (!triangles[curr].IsLeaf)
        {
            ...
            if (t.C0 >= 0 && Point_In_Triangle(pi, t.C0))
            {
                curr = t.C0;
            }
            else if (t.C1 >= 0 && Point_In_Triangle(pi, t.C1))
            {
                curr = t.C1;
            }
            else
    ...
    /// </summary>
    bool Point_In_Triangle(int pi, int ti)
    {
        var t = triangles[ti];
        return To_The_Left(pi, t.P0, t.P1)
            && To_The_Left(pi, t.P1, t.P2)
            && To_The_Left(pi, t.P2, t.P0);
    }

    /// <summary>
    /// Is the point to the left of the edge?
    /// </summary>
    bool To_The_Left(int pi, int li0, int li1)
    {
        if (li0 == -2)
        {
        ...
        else
        {
            ...
            return Geometry.To_The_Left(verts[pi], verts[li0], verts[li1]);
        }
    }
