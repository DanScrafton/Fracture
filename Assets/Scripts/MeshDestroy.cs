...
{
    private bool edge_Set = false;
    private Vector3 edge_Vertex = Vector3.zero;
    private Vector2 edge_UV = Vector2.zero;
    private Plane edge_Plane = new Plane();

    public int Cut_Cascades = 1;
    public float Explode_Force = 0;
    ...
    private void Destroy_Mesh()
    {
        ...
        for (var c = 0; c < Cut_Cascades; c++)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                var plane = new Plane(UnityEngine.Random.onUnitSphere, new Vector3(UnityEngine.Random.Range(bounds.min.x, bounds.max.x),
                 UnityEngine.Random.Range(bounds.min.y, bounds.max.y),UnityEngine.Random.Range(bounds.min.z, bounds.max.z)));


                subParts.Add(Generate_Mesh(parts[i], plane, true));
                subParts.Add(Generate_Mesh(parts[i], plane, false));
            }
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            parts[i].Make_Gameobject(this);
            parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * Explode_Force, transform.position);
        }

        Destroy(gameObject);
    }

    private PartMesh Generate_Mesh(PartMesh original, Plane plane, bool left)
    {
        ...
        partMesh.Fill_Arrays();

        return partMesh;
    }

    private void Add_Edge(int subMesh, PartMesh partMesh, Vector3 normal, Vector3 vertex1, Vector3 vertex2, Vector2 uv1, Vector2 uv2)
    {
        if (!edge_Set)
        {
            edge_Set = true;
        ...
        {
            edge_Plane.Set3Points(edge_Vertex, vertex1, vertex2);

            partMesh.Add_Triangle(subMesh,
                                edge_Vertex,
                                edge_Plane.GetSide(edge_Vertex + normal) ? vertex1 : vertex2,
                                edge_Plane.GetSide(edge_Vertex + normal) ? vertex2 : vertex1,
                                normal,
                                normal,
                                normal,
                                edge_UV,
    ...
    public class PartMesh
    {
        ...
        public void Add_Triangle(int submesh, Vector3 vert1, Vector3 vert2, Vector3 vert3, Vector3 normal1, Vector3 normal2, Vector3 normal3, Vector2 uv1, Vector2 uv2, Vector2 uv3)
        {
        ...
        public void Make_Gameobject(MeshDestroy original)
        {
            ...
            meshDestroy.Cut_Cascades = original.Cut_Cascades;
            meshDestroy.Explode_Force = original.Explode_Force;
