...
public class Breakable_Item : MonoBehaviour
{
    ...
    public void Reload()
    {
        ...
        var mesh = Mesh_From_Polygon(Polygon, Thickness);
    ...
    static float Normalized_Random(float mean, float stddev)
    {
    ...
    public void Break(Vector2 position)
    {
        ...
        {
            var calc = new Voronoi_Calculator();
            var clip = new Voronoi_Clipper();

            var sites = new Vector2[10];

            for (int i = 0; i < sites.Length; i++)
            {
                var dist = Mathf.Abs(Normalized_Random(0.5f, 1.0f / 2.0f));
            ...
            for (int i = 0; i < sites.Length; i++)
            {
                clip.Clip_Site(diagram, Polygon, i, ref clipped);
                ...
                if (clipped.Count > 0)
                {
                    ...
                    var bs = newGo.GetComponent<Breakable_Item>();
    ...
    static Mesh Mesh_From_Polygon(List<Vector2> polygon, float thickness)
    {
